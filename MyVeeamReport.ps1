...
If ($showJobsBk) {
  If ($allJobsBk.count -gt 0) {
    ...
    Foreach($bkJob in $allJobsBk) {
      $lastRun = ($bkJob.FindLastSession() | Select-Object -ExpandProperty EndTime)
      $nextRun = $bkJob.ScheduleOptions.NextRun
      $bodyJobsBk += $bkJob | Select-Object @{Name="Job Name"; Expression = {$_.Name}},
        ...
        @{Name="Last Run"; Expression = {$lastRun}}, # Déplacement de la colonne "Last Run" avant "Last Result"
        ...
        }} # Déplacement de la colonne "Next Run" à la fin
    }
...
If ($sendEmail) {
  ...
  If ($emailAttach) {
    $body = "Hello Team,
    In the attached, Veeam daily report for SGGH Prod Site :-)"
    $msg.Body = $body
